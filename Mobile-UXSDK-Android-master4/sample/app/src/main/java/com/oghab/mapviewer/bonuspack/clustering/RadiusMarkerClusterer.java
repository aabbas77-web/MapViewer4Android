...
//import com.oghab.mapviewer.bonuspack.R;
...
public class RadiusMarkerClusterer extends MarkerClusterer {
    ...
    protected double mMaxClusteringZoomLevel = 17;
    protected int mRadiusInPixels = 100;
    protected double mRadiusInMeters;
    protected Paint mTextPaint;
    private ArrayList<Overlay> mClonedMarkers;
    protected boolean mAnimated;
    int mDensityDpi;

    /** cluster icon anchor */
    public float mAnchorU = MyMarker.ANCHOR_CENTER, mAnchorV = MyMarker.ANCHOR_CENTER;
    /** anchor point to draw the number of markers inside the cluster icon */
    public float mTextAnchorU = MyMarker.ANCHOR_CENTER, mTextAnchorV = MyMarker.ANCHOR_CENTER;
    DisplayMetrics dm;
    ...
    public void setMaxClusteringZoomLevel(double zoom){
    ...
    @Override public ArrayList<StaticCluster> clusterer(MapView mapView) {
        ...
        mClonedMarkers = new ArrayList<Overlay>(mItems); //shallow copy
        while (!mClonedMarkers.isEmpty()) {
            Overlay m = mClonedMarkers.get(0);
    ...
    private StaticCluster createCluster(Overlay m, MapView mapView) {
//        GeoPoint clusterPosition = m.getPosition();
        GeoPoint clusterPosition = mv_utils.getPosition(m);
        ...
        Iterator<Overlay> it = mClonedMarkers.iterator();
        while (it.hasNext()) {
            Overlay neighbour = it.next();
//            double distance = clusterPosition.distanceToAsDouble(neighbour.getPosition());
            double distance = clusterPosition.distanceToAsDouble(mv_utils.getPosition(neighbour));
    ...
    @Override public MyMarker buildClusterMarker(StaticCluster cluster, MapView mapView) {
        MyMarker m = new MyMarker(mapView);
        ...
        mTextPaint.setTextSize(Tab_Map.map_text_size * dm.density);
        String text = "" + cluster.getSize();
        int textHeight = (int) (mTextPaint.descent() + mTextPaint.ascent());
        iconCanvas.drawText(text,
                mTextAnchorU * finalIcon.getWidth(),
                mTextAnchorV * finalIcon.getHeight() - textHeight / 2.0f,
                mTextPaint);
    ...
    @Override public void renderer(ArrayList<StaticCluster> clusters, Canvas canvas, MapView mapView) {
        for (StaticCluster cluster : clusters) {
            if (cluster.getSize() == 1) {
                ...
                //only draw 1 Overlay at Cluster center, displaying number of Markers contained
                Overlay m = buildClusterMarker(cluster, mapView);
                cluster.setMarker(m);
            }
        }
    }
