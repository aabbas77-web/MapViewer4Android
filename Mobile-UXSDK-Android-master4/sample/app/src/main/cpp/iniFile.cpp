...
string CIniFile::ValueName( string const keyname, unsigned const valueID) const
{
  try
  {
    ...
    if ( keyID == noID)
      return "";
    return ValueName(static_cast<const unsigned int>(keyID), valueID);
  }
  catch (exception ex)
  {
    return "";
...
bool CIniFile::SetValueI( string const keyname, string const valuename, int const value)
{
  try
  {
...
bool CIniFile::SetValueF( string const keyname, string const valuename, double const value)
{
  try
  {
    char svalue[MAX_VALUEDATA];
  			      ...
  			      void *v13, void *v14, void *v15, void *v16)
{
  try
  {
    ...
    nVals = static_cast<unsigned int>(sscanf(value.c_str(), format,
...
//    vector<AHhha>::iterator    kpos = keys.begin() + keyID;
